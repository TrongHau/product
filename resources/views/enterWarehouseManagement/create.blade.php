...
@section('content')
    <div class="breadcrumb">
        <nav class="bg-grey-light w-full rounded-md">
            <ol class="list-reset flex items-center">
                ...
                <li class="Inter-SemiBold text-lg tracking-[-0.02em] text-[#F20000]">Thêm mới nhập kho</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="mt-4 py-4">
        ...
        @endif
        <form class="" action="/create-enter_warehouse" method="POST" id="frm-enterWarehouse" enctype="multipart/form-data">
                ...
                <div class="flex flex-wrap -ml-2 -mr-2">
                            ...
                            <div class="mt-1">
                                <select data-te-select-init data-te-select-filter="true" id="warehouse" name="warehouse">
                                    @foreach($warehouseList as $item)
                                        <option {{(old('warehouse') == $item->id) ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="mt-1">
                                <select data-te-select-init data-te-select-filter="true" id="supplier" name="supplier">
                                    @foreach($supplierList as $item)
                                        <option {{(old('supplier') == $item->id) ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="mt-1">
                                <div class="relative">
                                    ...
                                    <input datepicker type="text" datepicker-format="yyyy-mm-dd" required id="purchase_date" name="purchase_date" value="{{old('purchase_date')}}" placeholder="yyyy-mm-dd" class="pl-10 w-full rounded border {{$errors->has('purchase_date') ? 'border-red-500' : 'border-gray-300'}} focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="p-2 mt-4 w-1/3">
                        <div class="relative">
                            ...
                            <input  type="text" id="code_receipt" name="code_receipt" value="{{old('code_receipt')}}" placeholder="Mã phiếu nhập hàng" class="mt-1 w-full rounded border {{$errors->has('code_receipt') ? 'border-red-500' : 'border-gray-300'}} focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                    ...
                    <div class="p-2 mt-4 w-1/3">
                        <div class="relative">
                            ...
                            <input  type="text" id="shipment" name="shipment" value="{{old('shipment')}}" placeholder="Nhập lo hàng" class="mt-1 w-full rounded border {{$errors->has('shipment') ? 'border-red-500' : 'border-gray-300'}} focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        </div>
                        ...
                        <div class="relative">
                            ...
                            <div class="mt-1">
                                <div class="relative">
                                    ...
                                    <input datepicker type="text" datepicker-format="yyyy-mm-dd" id="expired_date" required name="expired_date" value="{{old('expired_date')}}" placeholder="yyyy-mm-dd" class="pl-10 w-full rounded border {{$errors->has('expired_date') ? 'border-red-500' : 'border-gray-300'}} focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="p-2 mt-4 w-full">
                        <div class="relative">
                            ...
                            <div class="mt-1">
                                <textarea rows="2" name="note" placeholder="Ghi chú nhập kho" class="p-3 rounded w-full border">{{old('note')}}</textarea>
                            ...
                            <div class="flex flex-col w-full mt-4">
                                <div class="relative mb-4 flex flex-wrap items-stretch search_zone">
                                    ...
                                    <input
                                        ...
                                        autofocus
                                    />
                                    <label for="receiverCityV2" class="text-[#0D0F11] w-8 py-1 pl-2 pr-1 flex items-center rotate-180 cursor-pointer -ml-8">
                                ...
                                <div class="flex flex-wrap pt-4 pb-4 -mr-2 border-t product_item_{{$id}}">
                                    ...
                                    <div class="p-2" style="width: 18%">
                                        <div class="relative" style="width: 85%; display: inline-block">
                                            ...
                                            <div class="relative flex flex-wrap items-stretch mt-1">
                                                <input value="{{old('count_'.$id)}}" id="count_{{$id}}" name="count_{{$id}}" type="text" class="number_price count_{{$id}} relative m-0 block w-[1px] min-w-0 flex-auto rounded-l border border-solid border-neutral-300 bg-transparent bg-clip-padding px-3 py-[0.25rem] text-base font-normal leading-8 text-neutral-700 outline-none transition duration-200 ease-in-out focus:z-[3] focus:text-neutral-700 focus:outline-none" placeholder="Nhập số số lượng..." aria-label="Nhập số lượng..." aria-describedby="basic-addon2">
                                                <span class="flex items-center whitespace-nowrap rounded-r border border-l-0 border-solid border-neutral-300 px-3 py-[0.25rem] text-center text-base font-normal leading-8 text-neutral-700 dark:border-neutral-600 dark:text-neutral-200 dark:placeholder:text-neutral-200">VNĐ</span>
                                            </div>
