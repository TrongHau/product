...
@section('content')
    <div class="mt-4 py-2 pr-6 bg-[#F7F8F9]">
        <form class="gap-5 flex flex-wrap mb-0" id="frm-searching-v2" data-request="onSubmitSearch">
            <div class="relative xl:w-80 h-11" data-te-input-wrapper-init>
                ...
                <label for="name"
                       class="pointer-events-none absolute top-0 left-3 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.6rem] leading-[1.6] text-neutral-500 transition-all duration-200 ease-out peer-focus:-translate-y-[0.9rem] peer-focus:scale-[0.8] peer-focus:text-primary peer-data-[te-input-state-active]:-translate-y-[0.9rem] peer-data-[te-input-state-active]:scale-[0.8] motion-reduce:transition-none dark:text-neutral-200 dark:peer-focus:text-neutral-200">Tên kho hàng</label>
            </div>
            <div class="relative xl:w-[12.95rem] print-status h-11 bg-white rounded">
                <select data-te-select-init data-te-select-filter="true" id="status" name="status">
        ...
        @if(count($warehouseList) > 0)
            <div class="row mt-4">
                <div class="flex p-4 bottom-0 flex-grow justify-between">
                    ...
                    @if($search)
                        {{ $warehouseList->appends($search)->links('vendor.pagination.tailwind') }}
                    @else
                        {{ $warehouseList->links('vendor.pagination.tailwind') }}
                    @endif
                </div>
            </div>
        @endif
    ...
    <div
        ...
        <div
            ...
            <div class="min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none dark:bg-neutral-600">
                <form method="post" action="/warehouse-delete">
                    <div class="relative p-4 text-center bg-white rounded-lg  dark:bg-gray-800 sm:p-5">
                        ...
                        <p class="mb-4 text-gray-500 dark:text-gray-300">Bạn có chắc chắn xóa kho hàng này không?</p>
    ...
    <div
        ...
        <div
            ...
            <div class="min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none dark:bg-neutral-600">
                <form method="post" action="/create-warehouse">
                    <div class="relative p-4 text-center bg-white rounded-lg  dark:bg-gray-800 sm:p-5">
                        ...
                        <div class="p-5">
                            <label for="name" id="frm_title" class="leading-5 text-lg text-[#191D23] Inter-Regular font-bold -mt-[20px]">THÊM MỚI KHO HÀNG</label>
                        </div>
                        <div class="flex flex-wrap text-left">
                            <div class="p-2 w-1/2">
                                <div class="relative">
                                    <label for="frm_name" class="leading-5 text-base text-[#191D23] Inter-Regular">Tên kho hàng <span class="text-[#DC2626]">*</span></label>
                                    <input  type="text" id="frm_name" required name="name" value="" placeholder="Nhập tên kho hàng" class="mt-1 w-full rounded border focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                            <div class="p-2 w-1/2">
                                <div class="relative">
                                    <label for="frm_contact" class="leading-5 text-base text-[#191D23] Inter-Regular">Thông tin liên hệ</label>
                                    <input  type="text" id="frm_contact" name="contact" value="" placeholder="Nhập tên, sđt... tên của người liên hệ" class="mt-1 w-full rounded border focus:outline-none text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                </div>
                            </div>
                        </div>
                        <div class="flex flex-wrap text-left">
...
@section('scripts')
    <script>
        ...
        function createSupplier() {
            ...
            $('#frm_title').html('THÊM MỚI kho hàng');
            $('#createModal').modal('show');
        }
        $('.edit-user').on('click', function () {
            $.ajax({
                url: '/edit-warehouse/' + $(this).data('edit'),
                dataType: "json",
                type: "get",
                success: function (result) {
                    ...
                    $('#frm_title').html('CHỈNH SỬA kho hàng');
                    $('#createModal').modal('show');
                }
            });
        });
